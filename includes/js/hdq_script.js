...
const HDQ = {
	...
	VARS: {},
	init: async function () {
		console.log("Online Quiz Init");
		if (HDQ.EL.quizzes.length > 1) {
			for (let i = 0; i < HDQ.EL.quizzes.length; i++) {
				let html = `<p>Online QUIZ - WARNING: There is more than one quiz on this page. Due to the complexity of Online Quiz, only one quiz should on a page at a time.</p>`;
	...
	share: function () {
		function create_twitter_share() {
			...
			let hashtags = "&hashtags=OnlineQuiz";
